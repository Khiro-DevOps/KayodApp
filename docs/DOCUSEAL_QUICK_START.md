...
```
REDACTED_DOCUSEAL_API_KEY=xxx
DOCUSEAL_BASE_URL=https://api.docuseal.com
NEXT_PUBLIC_SUPABASE_URL=xxx
REDACTED_NEXT_PUBLIC_SUPABASE_ANON_KEY=xxx
REDACTED_SUPABASE_SERVICE_ROLE_KEY=xxx
NEXT_PUBLIC_APP_URL=http://localhost:3000  # or your domain
```

`REDACTED_DOCUSEAL_API_KEY` is required for creating submissions and generating the inline `embed_src` used by the signing UI.
