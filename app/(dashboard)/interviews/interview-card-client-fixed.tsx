...
export function InterviewCardClient({
  ...
  // ── Auto-close when duration exceeded ─────────────────────────────────────
  useEffect(() => {
    if (!isHR || interview.status === "completed" || interview.status === "cancelled") return;
    
    const checkAndShowPrompt = () => {
      if (isExpired && interview.status === "scheduled" && !showAutoClosePrompt) {
        setShowAutoClosePrompt(true);
      }
    };
    
    checkAndShowPrompt();
    const interval = setInterval(checkAndShowPrompt, 30000); // Check every 30 seconds
    return () => clearInterval(interval);
  }, [isExpired, interview.status, isHR, showAutoClosePrompt]);
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          // Auto-dismiss the "updated" indicator after 3 seconds
  ...
  const handleAutoClose = async () => {
    ...
    try {
      ...
      console.error("Error closing meeting room:", error);
    } finally {
      setSaving(false);
    }
  };
  ...
  // ── Auto-close prompt ────────────────────────────────────────────────────
  if (showAutoClosePrompt && isHR) {
    return (
      <div className="rounded-2xl bg-red-50 border border-red-200 p-5 space-y-4">
        <div>
          <h3 className="text-sm font-bold text-red-900">
            ⏱️ Meeting Time Exceeded
          </h3>
          <p className="text-xs text-red-800 mt-1">
            The scheduled meeting duration ({interview.duration_minutes} min) has passed. Would you like to close the meeting room and save your notes?
          </p>
        </div>
        <div className="flex gap-2">
          <button
            onClick={() => setShowAutoClosePrompt(false)}
            className="flex-1 rounded-xl border border-red-300 py-2.5 text-sm font-medium text-red-900 hover:bg-red-100 transition-colors"
          >
            Keep Open
          </button>
          <button
            ...
            disabled={saving}
            className="flex-1 rounded-xl bg-red-600 py-2.5 text-sm font-medium text-white hover:bg-red-700 transition-colors disabled:opacity-50"
          >
            {saving ? "Closing..." : "Close & Notes"}
          </button>
        </div>
      </div>
    );
  ...
  return (
    <div className={`rounded-2xl bg-surface border border-border p-4 space-y-3 ${past ? "opacity-60" : ""}`}>
      ...
      {hasUpdated && (
        <div className="rounded-xl bg-green-50 border border-green-200 px-3 py-2 text-xs text-green-700 animate-pulse">
          ✓ Interview updated — time or location changed
        </div>
      )}
