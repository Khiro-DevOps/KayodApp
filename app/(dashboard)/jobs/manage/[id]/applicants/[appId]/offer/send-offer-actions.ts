  ...
  try {
    ...
    if (!docusealTemplateId) {
      if (!process.env.REDACTED_DOCUSEAL_API_KEY) {
        return {
          error: "DocuSeal API key not configured. Please set REDACTED_DOCUSEAL_API_KEY in your environment variables.",
          success: false,
        };
      }
    ...
    const docusealResponse = await fetch(docusealUrl, {
      ...
      headers: {
        ...
        "X-Auth-Token": process.env.REDACTED_DOCUSEAL_API_KEY || "",
      },
